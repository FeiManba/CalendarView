...
public final class YearRecyclerView extends RecyclerView {
        ...
        mAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, long itemId) {
                if (mListener != null && mDelegate != null) {
                    ...
                    if (!com.feima.calendar.CalendarUtil.isMonthInRange(month.getYear(), month.getMonth(),
    ...
    final void init(int year) {
        ...
        for (int i = 1; i <= 12; i++) {
            date.set(year, i - 1, 1);
            int mDaysCount = com.feima.calendar.CalendarUtil.getMonthDaysCount(year, i);
            Month month = new Month();
            month.setDiff(com.feima.calendar.CalendarUtil.getMonthViewStartDiff(year, i, mDelegate.getWeekStart()));
    ...
    final void updateWeekStart() {
        for (Month month : mAdapter.getItems()) {
            month.setDiff(com.feima.calendar.CalendarUtil.getMonthViewStartDiff(month.getYear(), month.getMonth(), mDelegate.getWeekStart()));
        }
    }
