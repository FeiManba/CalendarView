...
public abstract class BaseWeekView extends BaseView {
     ...
     */
    final void setup(com.feima.calendar.Calendar calendar) {
        mItems = com.feima.calendar.CalendarUtil.initCalendarForWeekView(calendar, mDelegate, mDelegate.getWeekStart());
        addSchemesFromMap();
        invalidate();
    }
     ...
     * @param calendar calendar
     */
    final void setSelectedCalendar(com.feima.calendar.Calendar calendar) {
        if (mDelegate.getSelectMode() == com.feima.calendar.CalendarViewDelegate.SELECT_MODE_SINGLE &&
     ...
     */
    final void performClickCalendar(com.feima.calendar.Calendar calendar, boolean isNotice) {
        ...
        int week = com.feima.calendar.CalendarUtil.getWeekViewIndexFromCalendar(calendar, mDelegate.getWeekStart());
        if (mItems.contains(mDelegate.getCurrentDay())) {
            week = com.feima.calendar.CalendarUtil.getWeekViewIndexFromCalendar(mDelegate.getCurrentDay(), mDelegate.getWeekStart());
        }

        int curIndex = week;

        com.feima.calendar.Calendar currentCalendar = mItems.get(week);
        ...
        int i = com.feima.calendar.CalendarUtil.getWeekFromDayInMonth(currentCalendar, mDelegate.getWeekStart());
        mParentLayout.updateSelectWeek(i);


        if (mDelegate.mCalendarSelectListener != null
                && isNotice
                && mDelegate.getSelectMode() == com.feima.calendar.CalendarViewDelegate.SELECT_MODE_DEFAULT) {
     ...
     * @return 是否是最小访问边界了
     */
    final boolean isMinRangeEdge(com.feima.calendar.Calendar calendar) {
    ...
    final int getEdgeIndex(boolean isMinEdge) {
        for (int i = 0; i < mItems.size(); i++) {
            com.feima.calendar.Calendar item = mItems.get(i);
     ...
     * @return 获取点击的日历
     */
    protected com.feima.calendar.Calendar getIndex() {
    ...
    void updateCurrentDate() {
        if (mItems == null)
            return;
        if (mItems.contains(mDelegate.getCurrentDay())) {
            for (com.feima.calendar.Calendar a : mItems) {//添加操作
