...
public final class MonthViewPager extends ViewPager {
    ...
    com.feima.calendar.CalendarLayout mParentLayout;

    WeekViewPager mWeekPager;

    com.feima.calendar.WeekBar mWeekBar;
     ...
     */
    void setup(com.feima.calendar.CalendarViewDelegate delegate) {
    ...
    private void init() {
        ...
        addOnPageChangeListener(new OnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                if (mDelegate.getMonthViewShowMode() == com.feima.calendar.CalendarViewDelegate.MODE_ALL_MONTH) {
                    return;
                }
                int height;
            ...
            public void onPageSelected(int position) {
                com.feima.calendar.Calendar calendar = com.feima.calendar.CalendarUtil.getFirstCalendarFromMonthViewPager(position, mDelegate);
                ...
                if (mDelegate.getSelectMode() == com.feima.calendar.CalendarViewDelegate.SELECT_MODE_DEFAULT) {
                    if (!calendar.isCurrentMonth()) {
                        mDelegate.mSelectedCalendar = calendar;
                    } else {
                        mDelegate.mSelectedCalendar = com.feima.calendar.CalendarUtil.getRangeEdgeCalendar(calendar, mDelegate);
                    }
                ...
                if (!isUsingScrollToCalendar && mDelegate.getSelectMode() == com.feima.calendar.CalendarViewDelegate.SELECT_MODE_DEFAULT) {
                ...
                com.feima.calendar.BaseMonthView view = findViewWithTag(position);
                if (view != null) {
    ...
    private void updateMonthViewHeight(int year, int month) {
        if (mDelegate.getMonthViewShowMode() == com.feima.calendar.CalendarViewDelegate.MODE_ALL_MONTH) {//非动态高度就不需要了
        ...
        if (mParentLayout != null) {
            if (getVisibility() != VISIBLE) {//如果已经显示周视图，则需要动态改变月视图高度，否则显示就有bug
                ViewGroup.LayoutParams params = getLayoutParams();
                params.height = com.feima.calendar.CalendarUtil.getMonthViewHeight(year, month,
        ...
        mCurrentViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year, month,
                mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                mDelegate.getMonthViewShowMode());
        if (month == 1) {
            mPreViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year - 1, 12,
                    mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                    mDelegate.getMonthViewShowMode());
            mNextViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year, 2,
                    mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                    mDelegate.getMonthViewShowMode());
        } else {
            mPreViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year, month - 1,
                    mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                    mDelegate.getMonthViewShowMode());
            if (month == 12) {
                mNextViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year + 1, 1,
                        mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                        mDelegate.getMonthViewShowMode());
            } else {
                mNextViewHeight = com.feima.calendar.CalendarUtil.getMonthViewHeight(year, month + 1,
                        mDelegate.getCalendarItemHeight(), mDelegate.getWeekStart(),
                        mDelegate.getMonthViewShowMode());
            }
        }
    ...
    final void updateRange() {
        ...
        isUsingScrollToCalendar = false;
        com.feima.calendar.Calendar calendar = mDelegate.mSelectedCalendar;
        ...
        if (mParentLayout != null) {
            int week = com.feima.calendar.CalendarUtil.getWeekFromDayInMonth(calendar, mDelegate.getWeekStart());
    ...
    void scrollToCalendar(int year, int month, int day, boolean smoothScroll, boolean invokeListener) {
        ...
        com.feima.calendar.Calendar calendar = new com.feima.calendar.Calendar();
     ...
     * @return 获取当前月份数据
     */
    List<com.feima.calendar.Calendar> getCurrentMonthCalendars() {
        com.feima.calendar.BaseMonthView view = findViewWithTag(getCurrentItem());
        if (view == null) {
            return null;
        }
        return view.mItems;
    ...
    void updateSelected() {
        for (int i = 0; i < getChildCount(); i++) {
            com.feima.calendar.BaseMonthView view = (com.feima.calendar.BaseMonthView) getChildAt(i);
    ...
    void updateWeekStart() {
        ...
        setLayoutParams(params);
        if (mParentLayout != null) {
            int i = com.feima.calendar.CalendarUtil.getWeekFromDayInMonth(mDelegate.mSelectedCalendar, mDelegate.getWeekStart());
    ...
    private final class MonthViewPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(@NonNull ViewGroup container, int position) {
            ...
            com.feima.calendar.BaseMonthView view;
            try {
                Constructor constructor = mDelegate.getMonthViewClass().getConstructor(Context.class);
                view = (com.feima.calendar.BaseMonthView) constructor.newInstance(getContext());
            } catch (Exception e) {
                e.printStackTrace();
                return new com.feima.calendar.DefaultMonthView(getContext());
            }
            view.mMonthViewPager = com.feima.calendar.MonthViewPager.this;
