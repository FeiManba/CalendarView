...
final class CalendarUtil {
    ...
    static int getMonthViewLineCount(int year, int month, int weekStartWith, int mode) {
        if (mode == com.feima.calendar.CalendarViewDelegate.MODE_ALL_MONTH) {
            return 6;
        }
        int nextDiff = com.feima.calendar.CalendarUtil.getMonthEndDiff(year, month, weekStartWith);
        int preDiff = com.feima.calendar.CalendarUtil.getMonthViewStartDiff(year, month, weekStartWith);
        int monthDayCount = com.feima.calendar.CalendarUtil.getMonthDaysCount(year, month);
    ...
    static int getMonthViewStartDiff(Calendar calendar, int weekStart) {
        ...
        if (weekStart == com.feima.calendar.CalendarViewDelegate.WEEK_START_WITH_SUN) {
            return week - 1;
        }
        if (weekStart == com.feima.calendar.CalendarViewDelegate.WEEK_START_WITH_MON) {
            return week == 1 ? 6 : week - weekStart;
        }
        return week == com.feima.calendar.CalendarViewDelegate.WEEK_START_WITH_SAT ? 0 : week;
    }
     ...
     * @return 是否在日期范围内
     */
    static boolean isCalendarInRange(Calendar calendar, com.feima.calendar.CalendarViewDelegate delegate) {
    ...
    static List<Calendar> initCalendarForMonthView(int year, int month, Calendar currentDate, int weekStar) {
        ...
        if (month == 1) {//如果是1月
            ...
            nextYear = year;
            nextMonth = month + 1;
            preMonthDaysCount = mPreDiff == 0 ? 0 : com.feima.calendar.CalendarUtil.getMonthDaysCount(preYear, preMonth);
    ...
    static List<Calendar> getWeekCalendars(Calendar calendar, com.feima.calendar.CalendarViewDelegate mDelegate) {
     ...
     * @return 生成周视图的7个item
     */
    static List<Calendar> initCalendarForWeekView(Calendar calendar, com.feima.calendar.CalendarViewDelegate mDelegate, int weekStart) {
     ...
     */
    static Calendar getFirstCalendarFromMonthViewPager(int position, com.feima.calendar.CalendarViewDelegate delegate) {
        Calendar calendar = new Calendar();
        ...
        if (delegate.getDefaultCalendarSelectDay() != com.feima.calendar.CalendarViewDelegate.FIRST_DAY_OF_MONTH) {
     ...
     * @return 获取边界访问日期
     */
    static Calendar getRangeEdgeCalendar(Calendar calendar, com.feima.calendar.CalendarViewDelegate delegate) {
        if (com.feima.calendar.CalendarUtil.isCalendarInRange(delegate.getCurrentDay(), delegate)
                && delegate.getDefaultCalendarSelectDay() != com.feima.calendar.CalendarViewDelegate.LAST_MONTH_VIEW_SELECT_DAY_IGNORE_CURRENT) {
     ...
     * @return 是否是最小访问边界了
     */
    private static boolean isMinRangeEdge(Calendar calendar, com.feima.calendar.CalendarViewDelegate delegate) {
        java.util.Calendar c = java.util.Calendar.getInstance();
