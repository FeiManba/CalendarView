...
import com.feima.calendarlibrary.R;
...
public final class LunarCalendar {
    ...
    static void init(Context context) {
        ...
        com.feima.calendar.TrunkBranchAnnals.init(context);
        com.feima.calendar.SolarTermUtil.init(context);
    ...
    public static int daysInLunarMonth(int year, int month) {
        if ((LUNAR_INFO[year - com.feima.calendar.CalendarViewDelegate.MIN_YEAR] & (0x10000 >> month)) == 0)
            return 29;
        else
            return 30;
    }
    ...
    private static String getSolarTerm(int year, int month, int day) {
        if (!SOLAR_TERMS.containsKey(year)) {
            SOLAR_TERMS.put(year, com.feima.calendar.SolarTermUtil.getSolarTerms(year));
        }
    ...
    public static String getLunarText(int year, int month, int day) {
        String termText = com.feima.calendar.LunarCalendar.getSolarTerm(year, month, day);
        String solar = com.feima.calendar.LunarCalendar.gregorianFestival(month, day);
        if (!TextUtils.isEmpty(solar))
            return solar;
        if (!TextUtils.isEmpty(termText))
            return termText;
        int[] lunar = com.feima.calendar.LunarUtil.solarToLunar(year, month, day);
        String festival = getTraditionFestival(lunar[0], lunar[1], lunar[2]);
        if (!TextUtils.isEmpty(festival))
            return festival;
        return com.feima.calendar.LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]);
    }
     ...
     */
    public static void setupLunarCalendar(com.feima.calendar.Calendar calendar) {
        int year = calendar.getYear();
        int month = calendar.getMonth();
        int day = calendar.getDay();
        calendar.setWeekend(com.feima.calendar.CalendarUtil.isWeekend(calendar));
        calendar.setWeek(com.feima.calendar.CalendarUtil.getWeekFormCalendar(calendar));

        com.feima.calendar.Calendar lunarCalendar = new com.feima.calendar.Calendar();
        ...
        calendar.setLeapYear(com.feima.calendar.CalendarUtil.isLeapYear(year));
        if (lunar[3] == 1) {//如果是闰月
            calendar.setLeapMonth(lunar[1]);
            lunarCalendar.setLeapMonth(lunar[1]);
        }
        String solarTerm = com.feima.calendar.LunarCalendar.getSolarTerm(year, month, day);
        String gregorian = com.feima.calendar.LunarCalendar.gregorianFestival(month, day);
        String festival = getTraditionFestival(lunar[0], lunar[1], lunar[2]);
        String lunarText = com.feima.calendar.LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]);
     ...
     * @return 获取农历节日
     */
    public static String getLunarText(com.feima.calendar.Calendar calendar) {
